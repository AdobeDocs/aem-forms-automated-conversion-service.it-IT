...
1. **Quale versione di AEM Forms è supportata dal servizio di conversione automatica dei moduli (AFCS)?**
   <p>Il servizio di conversione automatica dei moduli (AFCS) supporta AEM 6.5 Forms, AEM 6.5 LTS Forms e AEM Forms as a Cloud Service. Funziona sia con AEM Forms su OSGi che con AEM Forms su JEE. Per AEM 6.5 e AEM 6.5 LTS, è necessario disporre del pacchetto del componente aggiuntivo AEM Forms più recente oltre all'istanza di authoring di AEM e <a href="https://experienceleague.adobe.com/docs/experience-manager-65/forms/adaptive-forms-core-components/enable-adaptive-forms-core-components.html?lang=it">abilitare i componenti core del modulo adattivo</a> se si desidera utilizzare modelli e temi basati su componenti core. Per istruzioni dettagliate, consulta <a href="configure-service.md">Configurare il servizio Automated Forms Conversion</a>.</p> 
    <br>
...
1. **Il servizio può produrre un XDP invece di un modulo adattivo?**
   <p>Il servizio non produce un output XDP. Aggiungiamo regolarmente funzionalità e al servizio. Mantieni la sezione <a href="introduction.md">lingue supportate e PDF forms</a> nella tua watchlist per un aggiornamento regolare sulle nuove funzionalità e funzionalità aggiunte.</p> <br>
...
1. **Il servizio può convertire i moduli cartacei digitalizzati e i moduli colorati in moduli adattivi?**
   <p>Il servizio può convertire PDF forms a colori in moduli adattivi. Il servizio non supporta moduli digitalizzati o compilati. Per altre limitazioni, consulta l'articolo <a href="known-issues.md">problemi noti</a>.</p> <br>
...
I motivi più comuni per cui la conversione non riesce sono:</p>
   * Per la conversione vengono forniti PDF forms protetti. Non utilizzare PDF forms protetto da password o protetto per la conversione.
   * Connessione Internet interrotta. Assicurati di essere connesso a Internet durante la conversione.
   * In Source PDF è disponibile un&#39;immagine del modulo anziché del modulo effettivo.
...
1. **L&#39;utilizzo di tipi di carattere personalizzati influisce sulla conversione?**
   <p>Quando un modulo PDF non interattivo viene convertito in un modulo adattivo, per migliorare la qualità della conversione, i font vengono incorporati nel modulo PDF. Il supporto per l'incorporamento di tipi di carattere è limitato a PDF forms non interattivo. Per ottimizzare la conversione di AcroForm e PDF forms basati su XFA, vengono utilizzati font di fallback.</p> 
    <p>Solo i moduli disponibili nella directory dei font personalizzati elencata nel campo <strong>Directory dei font del cliente</strong> della configurazione del servizio Gestione font <strong> CQ-DAM-Handler-Gibson</strong> sono incorporati in un modulo PDF non interattivo.</p> 
    <p> </p> <br>
...
L&#39;errore sopra riportato si verifica quando la delega di avvio non è configurata per le librerie RSA/BouncyCastle. Per risolvere il problema, effettua le seguenti operazioni:
   ...
   1. Arresta l’istanza di AEM. Passare alla cartella `[AEM installation directory]\crx-quickstart\conf\`. Apri il file sling.properties per la modifica. Se utilizzi `[AEM installation directory]\crx-quickstart\bin\start.bat` per avviare un&#39;istanza di AEM, modifica il file sling.properties che si trova in `[AEM_root]\crx-quickstart\`.
...
1. **Posso usare i tag di testo di Adobe Sign con il servizio di conversione automatica dei moduli (AFCS)?**
   <p> Quando si utilizza il servizio AFCS (Automated Forms Conversion Service) per convertire un modulo di PDF in un modulo adattivo e il modulo di PDF dispone di tag di testo Adobe Sign, tali tag vengono convertiti nei campi del modulo adattivo corrispondenti e i dettagli del firmatario vengono compilati automaticamente.  La funzione è disponibile solo per Acro Forms e i moduli adattivi supportano un numero limitato di campi Adobe Sign.</p>  </br>
